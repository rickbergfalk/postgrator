...
module.exports = function(config) {
  if (supportedDrivers.indexOf(config.driver) === -1) {
    throw new Error(
      'db driver is not supported. Must either be ' +
        supportedDrivers.join(' or ') +
        '.'
    )
  }
  ...
  if (config.driver === 'mysql') {
    ...
    commonClient.queries.makeTable =
      'CREATE TABLE ' +
      config.schemaTable +
      ' (version BIGINT, PRIMARY KEY (version)); INSERT INTO ' +
      config.schemaTable +
      ' (version) VALUES (0);'
    ...
    commonClient.queries.checkTable =
      "SELECT * FROM pg_catalog.pg_tables WHERE schemaname = CURRENT_SCHEMA AND tablename = '" +
      config.schemaTable +
      "';"
    commonClient.queries.makeTable =
      'CREATE TABLE ' +
      config.schemaTable +
      " (version BIGINT PRIMARY KEY, name TEXT DEFAULT '', md5 TEXT DEFAULT ''); INSERT INTO " +
      config.schemaTable +
      " (version, name, md5) VALUES (0, '', '');"
    ...
    commonClient.queries.makeTable =
      'CREATE TABLE ' +
      config.schemaTable +
      ' (version BIGINT PRIMARY KEY); INSERT INTO ' +
      config.schemaTable +
      ' (version) VALUES (0);'

    commonClient.createConnection = function(cb) {
      commonClient.dbConnection = commonClient.dbDriver.connect(
        sqlconfig,
        function(err) {
          cb(err)
        }
